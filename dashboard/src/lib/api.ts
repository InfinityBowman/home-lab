...
// System
export const getHealth = () => request<{ status: string; version: string }>("/system/health");
export const getSystemInfo = () =>
  request<{ version: string; docker_version: string; app_count: number; container_count: number }>(
    "/system/info",
  );

// Apps
export const listApps = () =>
  request<import("@/types/api").App[]>("/apps");
export const getApp = (name: string) =>
  request<import("@/types/api").App>(`/apps/${name}`);
export const createApp = (data: { name: string; port?: number }) =>
  request<import("@/types/api").App>("/apps", {
    method: "POST",
    body: JSON.stringify(data),
  });
export const updateApp = (name: string, data: { port?: number; domain?: string }) =>
  request<import("@/types/api").App>(`/apps/${name}`, {
    method: "PUT",
    body: JSON.stringify(data),
  });
...
export const getContainerStatus = (name: string) =>
  request<import("@/types/api").ContainerStatus>(`/apps/${name}/status`);
...
export const triggerDeploy = (name: string) =>
  request<{ deployment_id: string; status: string }>(`/apps/${name}/deploy`, { method: "POST" });
export const listDeployments = (name: string) =>
  request<import("@/types/api").Deployment[]>(`/apps/${name}/deployments`);
export const getDeployment = (name: string, id: string) =>
  request<import("@/types/api").Deployment>(`/apps/${name}/deployments/${id}`);
...
export const listEnvVars = (name: string) =>
  request<import("@/types/api").MaskedEnvVar[]>(`/apps/${name}/env`);
